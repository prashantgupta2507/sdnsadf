...
export default function ManageAddresses() {
    ...
    const { getAddresses, updateAddrComState } = bindActionCreators(addressActionCreators,dispatch)

    useEffect(() => {
      getAddresses();
      if (query.get("ref")) {
        updateAddrComState(true);
      }
    }, [dispatch]);
      ...
      <>
        <Box className={classes.component}>
          ...
          {openAddAddress ? (
            ...
            <Box
              className={clsx(classes.card, classes.addAddress)}
              onClick={() => updateAddrComState(true)} //Update Address Component State
            >
